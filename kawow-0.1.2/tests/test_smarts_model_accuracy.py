...
def _predict_logkow(calc: NaefAcreePartitionCalculator, mol) -> float:
    ...
    raise RuntimeError("Could not extract logKow from SMARTS model output")


def test_smarts_logkow_accuracy_on_s01(smarts_calc):
    """
    Validate SMARTS-model logKow accuracy against S01 experimental logP values.
    ...
    assert coverage >= 0.90, f"SMARTS model coverage too low on S01: {coverage:.3f}"
    ...
    assert r2 >= 0.85, (
        f"SMARTS logKow R2 below target on S01: {r2:.3f} "
