...
Fitted coefficients are saved as JSON in partcoeff/data/ so the package can
...
def fit(
    ...
    coefficients to partcoeff/data/*.json.
...
def _load_json(path: Path) -> dict:
    if not path.exists():
        raise FileNotFoundError(
            ...
            "Run partcoeff.fit(sdf_logkow, sdf_logkoa) first."
        )
    with open(path) as f:
        return json.load(f)
...
class PartitionCalculator:
    ...
    Usage
    -----
    >>> from partcoeff import PartitionCalculator
